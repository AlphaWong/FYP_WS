...
public class WebSocketMessageInbound extends MessageInbound {
    ...
    protected void onTextMessage(CharBuffer message) throws IOException {
        //向所有在线用户发送消息
        boolean broadcast = message.toString().contains(",\"to\":\"all\"");
        System.out.println("broadcast : " + broadcast);
        if (broadcast == true) {
            WebSocketMessageInboundPool.sendMessage(message.toString());
        } else {
            JsonObject jo=new Gson().fromJson(message.toString(), JsonObject.class);
            String from=jo.get("from").toString().replaceAll("\"","");
            String to=jo.get("to").toString().replaceAll("\"","");
            //System.out.println("***["+jo.get("to").toString().replaceAll("\"","")+"]");
